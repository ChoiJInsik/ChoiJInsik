	...
	<main role="main">
		...
		<div class="album py-5 bg-light">
			<div align="center" class="btn-group-center mt-3 mb-5" role="group"
				...
				<a href="${contextPath}/yook/GoodsListAll.do"><button
						type="button" class="btn btn-secondary">전체보기</button></a> <a
					href="${contextPath}/yook/GoodsListPig.do"><button
						type="button" class="btn btn-secondary">돼지고기</button></a> <a
					href="${contextPath}/yook/GoodsListCow.do"><button
						type="button" class="btn btn-secondary">소고기</button></a> <a
					href="${contextPath}/yook/GoodsListChicken.do"><button
						type="button" class="btn btn-secondary">닭고기</button></a>
			</div>
								...
								<c:forEach items="${list}" var="list">
									...
									<div>
										...
										<a href="#this" name="title" style="text-decoration: none;">
											...
											<div class="card mb-4"
												style="margin-left: 20px; border: none; color: black; background-color: whitesmoke; width: 330px;">
												...
												<input type="hidden" id="GOODS_NUM" name="GOODS_NUM"
													value="${list.GOODS_NUM}" /> <font size="4em"
													class="font1" style="margin-left: 9px; margin-bottom: 5px;"><strong>${list.GOODS_NAME}
												</strong></font> <font class="font3"
													style="margin-left: 9px; margin-bottom: 15px; color: gray;"><fmt:formatNumber
														value="${list.GOODS_PRICE}" pattern="#,###">
													</fmt:formatNumber>원/${list.GOODS_WEIGHT } <br /> <c:if
														test="${list.GOODS_WEIGHT == '300g(14mm)' }">
							(100g당 ${list.GOODS_PRICE / 3}원)
							</c:if> <c:if test="${list.GOODS_WEIGHT == '300g(16mm)' }">
							(100g당 ${list.GOODS_PRICE / 3}원)
							</c:if> <c:if test="${list.GOODS_WEIGHT == '300g(18mm)' }">
							(100g당 ${list.GOODS_PRICE / 3}원)
							</c:if> <c:if test="${list.GOODS_WEIGHT == '300g' }">
							(100g당 ${list.GOODS_PRICE / 3}원)
							</c:if> <c:if test="${list.GOODS_WEIGHT == '200g' }">
							(100g당 ${list.GOODS_PRICE / 2}원)
							</c:if> <c:if test="${list.GOODS_WEIGHT == '150g' }">
